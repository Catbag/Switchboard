 ...
 * <p>
 * For usage <code>initDefaultServerUrls</code> for first time usage. Server URLs can be updates
 * from
 * a remote location with <code>initConfigServerUrl</code>.
 * <p>
 * To run a experiment use <code>isInExperiment()</code>. The experiment name has to match the
 * one you
 * setup on the server.
 * All functions are design to be safe for programming mistakes and network connection issues. If
 * the
...
public class SwitchBoard {
    ...
    /**
     * Set if the application is run in debug mode. DynamicConfig runs against staging server
     * when in debug and production when not
     */
    public static boolean DEBUG = true;
    /**
     * Production server to update the remote server URLs. http://staging
     * .domain/path_to/SwitchboardURLs.php
     */
    private static String DYNAMIC_CONFIG_SERVER_URL_UPDATE;
    /**
     * Production server for getting the actual config file. http://staging
     * .domain/path_to/SwitchboardDriver.php
     */
     ...
     * @param configServerUrl       Url to: http://staging.domain/path_to/SwitchboardDriver.php -
     *                              the acutall config
     * @param isDebug               Is the application running in debug mode. This will add log
     *                              messages.
     */
    ...
    /**
     * Advanced initialization that supports a production and staging environment without
     * changing the server URLs manually.
     * SwitchBoard will connect to the staging environment in debug mode. This makes it very
     * simple to test new experiements
     * during development.
     *
     * @param configServerUpdateUrlStaging Url to http://staging.domain/path_to/SwitchboardURLs
     *                                     .php in staging environment
     * @param configServerUrlStaging       Url to: http://staging
     *                                     .domain/path_to/SwitchboardDriver.php in staging - the
     *                                     acutall config
     * @param configServerUpdateUrl        Url to http://staging.domain/path_to/SwitchboardURLs
     *                                     .php in production environment
     * @param configServerUrl              Url to: http://staging
     *                                     .domain/path_to/SwitchboardDriver.php in production -
     *                                     the acutall config
     * @param isDebug                      Defines if the app runs in debug.
     */
    public static void initDefaultServerUrls(String configServerUpdateUrlStaging, String
    ...
    /**
     * Set HTTP connection and read timeouts to be used on <code>loadConfig</code> and
     * <code>updateConfigServerUrl</code>.
     *
    ...
    /**
     * Updates the server URLs from remote and stores it locally in the app. This allows to move
     * the server side
    ...
    public static void updateConfigServerUrl(Context c) {
        ...
        if (DEBUG) {
            ...
            Preferences.setDynamicConfigServerUrl(c, DYNAMIC_CONFIG_SERVER_URL_UPDATE,
        ...
        try {
            ...
            if (result != null) {
                ...
                Preferences.setDynamicConfigServerUrl(c, (String) a.get(kUpdateServerUrl),
                        (String) a.get(kConfigServerUrl));
    ...
    /**
     * Loads a new config file for the specific user from current config server. Uses internal
     * unique user ID.
     * Use this method only in background thread as network connections are involved that block
     * UI thread.
    ...
    /**
     * Loads a new config for a user. This method allows you to pass your own unique user ID
     * instead of using
    ...
    public static void loadConfig(Context c, String uuid) {
        ...
        try {
            ...
            try {
                versionName = c.getPackageManager().getPackageInfo(c.getPackageName(), 0)
                        .versionName;
            ...
            String params = "uuid=" + uuid + "&device=" + device + "&lang=" + lang + "&country="
                    + country
                    + "&manufacturer=" + manufacturer + "&appId=" + packageName + "&version=" +
                    versionName
                    + "&sdkVersion=" + sdkVersion;
            if (DEBUG) Log.d(TAG, "Read from server URL: " + serverUrl + params);
            String serverConfig = readFromUrlGET(serverUrl, params, mConnectionTimeout,
                    mReadTimeout);
    ...
    /**
     * Looks up in config if user is in certain experiment. Returns false as a default value when
     * experiment
     * does not exist.
     ...
     */
    public static boolean isInExperiment(Context c, String experimentName, boolean
            defaultReturnVal) {
        ...
        if (config == null)
            ...
            try {
                ...
                if (DEBUG)
                    Log.d(TAG, "experiment " + experimentName + " JSON object: " + experiment
                            .toString());
                if (experiment == null)
    ...
    /**
     * Returns the experiment value as a JSONObject. Depending on what experiment is has to be
     * converted to the right type.
     * Typcasting is by convention. You have to know what it's in there. Use <code>hasExperiment
     * ()</code>
     ...
     */
    private static String readFromUrlGET(String address, String params, int connectionTimeout,
                                         int readTimeout) {
